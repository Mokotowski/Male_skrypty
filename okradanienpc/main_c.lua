...
function robNpc(targetPed)
    ...
    ESX.Game.Utils.DrawText3D(roblocalcoords, "[~g~E~s~] zeby Okrasc", 0.25)
    elseif lasttargetPed == targetPed then
    ESX.Game.Utils.DrawText3D(roblocalcoords, "Okradanie...", 0.25)
    else
        ...
        if IsControlJustReleased(0, 38) then
            ...
            if not currentrobbing then
                if lasttargetPed == targetPed then
                    ...
                    ESX.ShowNotification("Juz okradles osobe")
                       ...
                       if not IsPedDeadOrDying(targetPed) then
                            ...
                            if Config.AlwaysNotifyonDeath then
                                ...
                                ESX.TriggerServerCallback('esx_skin:getPlayerSkin', function(skin, jobSkin)
                                    local sex = nil
                                    if skin.sex == 0 then
                                        sex = "Mezczyzna" --male/change it to your language
                                    else
                                        sex = "Kobieta" --female/change it to your language
                                    end
                                    TriggerServerEvent('esx_mugging:muggingPos', plyPos.x, plyPos.y, plyPos.z)
...
AddEventHandler('muggingNotify', function(alert, xPlayer, coords)
        if  ESX.PlayerData.job.name == 'police' then  
            if Config.GCPhone then 
                ...
                ESX.ShowAdvancedNotification('Policja', 'Okradanie', alert, 'CHAR_CALL911', 1)
            end
        end
end)
