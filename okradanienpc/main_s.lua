...
AddEventHandler('esx_mugging:giveMoney', function()
    ...
    TriggerClientEvent("esx:showNotification", source, ("~y~Ukradles ~g~$%s"):format(amount))
end)
...
AddEventHandler('esx_mugging:muggingAlert', function(street1, street2, sex, coords)
    if Config.GCPhone  then
        TriggerClientEvent("muggingNotify", -1, "Zgloszono kradziez przez "..sex.." near "..street1.." i "..street2, coords)
    else
	    TriggerClientEvent("muggingNotify", -1, "~r~Zgloszono kradziez przez ~w~ "..sex.." ~r~blisko~w~ "..street1.." i "..street2, coords)
    end
end)
