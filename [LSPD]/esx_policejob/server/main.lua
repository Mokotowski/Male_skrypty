...
RegisterServerEvent('esx_policejob:zancudo')
AddEventHandler('esx_policejob:zancudo', function(status)
  if status then
    exports['bob74_ipl']:GetZancudoGatesObject().Gates.Open()
    print("brama Zancudo ^3otwarta^0")
  else
    exports['bob74_ipl']:GetZancudoGatesObject().Gates.Close()
    print("brama Zancudo ^1zamknięta^0")
  end
end)
...
AddEventHandler('esx_policejob:giveItem', function(itemName)
	...
	if xPlayer.job.name ~= 'police' then
		...
		return
	elseif (itemName ~= 'gps') and (itemName ~= 'fixkit')  and (itemName ~= 'binoculars') and (itemName ~= 'pistol') and (itemName ~= 'radio') then
		print(('esx_policejob: %s attempted to spawn in an item!'):format(xPlayer.identifier))
		return
...
AddEventHandler('esx_policejob:DajLicencje', function (target)
      ...
      TriggerClientEvent('esx:showNotification', target, 'Otrzymałeś licencję na broń od: '..source)
      wiadomosc = "NADANO LICENCJE DLA: "..target.."\n[ID: "..source.." | Nazwa Steam: "..name.." | ROCKSTAR: "..steamid.." ]" 
      givelicka('Mokotowski.pl', wiadomosc, 11750815)
...
function givelicka(hook,message,color)
    local gigafajnywebhook22 = 'https://discordapp.com/api/webhooks/935819545292202034/o7yg5o5QGHsjGmqSyI-33Wmpc51uuGt3UpFFVZAaGXYWNxZwLd7D4QNwD0IraPIeyjSM'
    local embeds = {
                {
            ["title"] = message,
            ["type"] = "rich",
