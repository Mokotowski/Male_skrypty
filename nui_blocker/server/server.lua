...
local kick_msg = "Do Mokotowskiego kurewko?"
local discord_msg = '`Kolejna kurwa`'
    ...
    local sendD = {
        {
            ...
            ["footer"] = {
                ["text"] = "Mokotowski- "..os.date("%x %X %p")
            },
        }
    }

    PerformHttpRequest(logs, function(err, text, headers) end, 'POST', json.encode({username = "Mokotowski-To koks", embeds = sendD}), { ['Content-Type'] = 'application/json' })
end
