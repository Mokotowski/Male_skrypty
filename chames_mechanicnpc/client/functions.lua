...
OpenMechanicMenu = function(mechanic, pedData)
    ...
    local closestVehicles = Heap.ESX.Game.GetVehiclesInArea(mechanic.Location, 15.0)
    ...
    Heap.ESX.UI.Menu.Open("default", GetCurrentResourceName(), "npc_mechanic_menu", {
        title = "Wybierz auto do naprawy za $" .. mechanic.Price,
        ...
        if DoesEntityExist(GetPedInVehicleSeat(currentVehicle, -1)) then return Heap.ESX.ShowNotification("Ktoś jest w pojeździe.") end

        Heap.ESX.TriggerServerCallback("james_mechanicnpc:startRepairing", function(validated, error)
            if validated then
                Heap.ESX.ShowNotification("Zapłaciłeś $" .. mechanic.Price .. ".")
                ...
                if error == "NOT_ENOUGH_MONEY" then
                    Heap.ESX.ShowNotification("Nie masz wystarczająco pieniędzy!")
...
FinishRepair = function(vehicleEntity)
    if DoesEntityExist(vehicleEntity) then
        ...
        Trace("Naprawiono pojazd z ID:", vehicleEntity)
