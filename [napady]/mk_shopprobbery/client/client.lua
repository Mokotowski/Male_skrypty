		...
		if not blockstart then
			if(GetDistanceBetweenCoords(coords,Config.shoprobbery['BossSpawn'].Pos.x, Config.shoprobbery['BossSpawn'].Pos.y, Config.shoprobbery['BossSpawn'].Pos.z, true) < 9.0) and not started then	
				...
				if(GetDistanceBetweenCoords(coords,Config.shoprobbery['BossSpawn'].Pos.x, Config.shoprobbery['BossSpawn'].Pos.y, Config.shoprobbery['BossSpawn'].Pos.z, true) < 1.5) then	
					...
					DrawText3Ds(Config.shoprobbery['BossSpawn'].Pos.x, Config.shoprobbery['BossSpawn'].Pos.y, Config.shoprobbery['BossSpawn'].Pos.z+1.0, 'Aby zacząć napad, Naciśnij [~g~E~w~]')
					if IsControlJustReleased(0, Keys['E']) and not IsPedInAnyVehicle(ped, false) then
							...
							if PoliceAmount >= Config.Police then
								...
								ESX.TriggerServerCallback('inside-shoprobbery:SecBetwNextRob', function(YouCan)	
									if YouCan then
										exports.pNotify:SendNotification({text = "<b>Napad na sklep</b></br>Zbierz informacje o ilości pieniędzy w sklepach od menadżera", timeout = 4000})
										...
										AddTextComponentString('Menadżer sklepu')
										EndTextCommandSetBlipName(BlipCodeNPC)
										SetBlipRoute(BlipCodeNPC, true)
									else
										exports.pNotify:SendNotification({text = "<b>Napad na sklep</b></br>Czekaj na swoje zadanie", timeout = 4000})
									end
								end)

							else
								exports.pNotify:SendNotification({text = "<b>Napad na sklep</b></br>W mieście jest za mało policji", timeout = 4000})
							end
			...
			if(GetDistanceBetweenCoords(coords,Config.shoprobbery['BossSpawn'].Pos.x, Config.shoprobbery['BossSpawn'].Pos.y, Config.shoprobbery['BossSpawn'].Pos.z, true) < 9.0) then	
				...
				if(GetDistanceBetweenCoords(coords,Config.shoprobbery['BossSpawn'].Pos.x, Config.shoprobbery['BossSpawn'].Pos.y, Config.shoprobbery['BossSpawn'].Pos.z, true) < 1.5) then	
					...
					if started then
						DrawText3Ds(Config.shoprobbery['BossSpawn'].Pos.x, Config.shoprobbery['BossSpawn'].Pos.y, Config.shoprobbery['BossSpawn'].Pos.z+1.0, 'Aby anulować zlecenie, naciśnij [~r~E~w~]')
						if IsControlJustReleased(0, Keys['E']) and not IsPedInAnyVehicle(ped, false) then
							DeletePed(CodeNPC)
						...
						DrawText3Ds(Config.shoprobbery['BossSpawn'].Pos.x, Config.shoprobbery['BossSpawn'].Pos.y, Config.shoprobbery['BossSpawn'].Pos.z+1.0, 'Ktoś mi właśnie zleca')		
					end			
				else
	...
	while true do
		if started then
			...
			if started and not CashRegister then
				exports.pNotify:SendNotification({text = "<b>Napad na sklep</b></br>Czas na wykonanie zadania upłynął", timeout = 4000})
				DeletePed(CodeNPC)
				pedwalking = false
				RemoveBlip(ShopBlip)
				DeletePed(ShopNPC)
	...
	while true do
		...
		if started then
			...
			if(GetDistanceBetweenCoords(coords,NPCcoords.x, NPCcoords.y, NPCcoords.z, true) < 1.5) then	
				sleep = 5
				DrawText3Ds(NPCcoords.x, NPCcoords.y, NPCcoords.z+1.0, 'By zebrać dokumenty, naciśnij [~r~E~w~]')
				if IsControlJustReleased(0, Keys['E']) and not IsPedInAnyVehicle(ped, false) then
					pedwalking = false
					ClearPedTasks(CodeNPC)
					...
					exports['progressBarss']:startUI((3000), "Przeszukujesz")	
					Citizen.Wait(3000)
					...
					exports.pNotify:SendNotification({text = "<b>Napad na sklep</b></br>Znalazłeś udaj się na "..Shop.name, timeout = 4000})
					ShopRobbery()
				end
			end
		end	
...
AddEventHandler('inside-shoprobbery:PoliceNotifyC', function(coords)
	if PlayerData.job ~= nil and PlayerData.job.name == 'police' then
		...
		exports.pNotify:SendNotification({text = "<b>Napad na sklep</b></br>Ktoś wyjął broń w sklepie <b>" ..street2.. "</b>.", timeout = 7500})
		...
		BeginTextCommandSetBlipName("STRING")
		AddTextComponentString('Napad na sklep')
	...
	while true do
		...
		local coords = GetEntityCoords(ped)
			if(GetDistanceBetweenCoords(coords, Config.Shops[Shop.name].PedPos.x, Config.Shops[Shop.name].PedPos.y, Config.Shops[Shop.name].PedPos.z, true) < 6.5) then	
				...
				DrawText3Ds(Config.Shops[Shop.name].PedPos.x, Config.Shops[Shop.name].PedPos.y, Config.Shops[Shop.name].PedPos.z+1.0, 'Dzień dobry, Jak pomóc?')
				if IsPlayerFreeAiming(PlayerId()) then
					...
					if IsPedFleeing(targetPed) and targetPed == ShopNPC then						
						...
						while displaying do
							...
							DrawText3Ds(coordsPed['x'], coordsPed['y'], coordsPed['z'] + 1.2, "Już otwieram")
						end
						atlocation = false
						Citizen.CreateThread(function()
							Wait(10000)
						...
						Citizen.CreateThread(function()
							...
							while true do
								...
								if Config.Shops[Shop.name].Time <= 0 then
									exports.pNotify:SendNotification({text = "<b>Napad na sklep</b></br>Gotówka gotowa do odbioru", timeout = 4500})
									ClearPedTasks(ShopNPC)
						...
						while true do
							...
							if Config.Shops[Shop.name].Time > 0 then
								if(GetDistanceBetweenCoords(coords, Config.Shops[Shop.name].PedWalks.x, Config.Shops[Shop.name].PedWalks.y, Config.Shops[Shop.name].PedWalks.z, true) < 3.5) then
									DrawText3Ds(Config.Shops[Shop.name].PedWalks.x, Config.Shops[Shop.name].PedWalks.y, Config.Shops[Shop.name].PedWalks.z+1.0, 'Czas: '..tostring(Config.Shops[Shop.name].Time))
								end
								...
								if(GetDistanceBetweenCoords(coords, Config.Shops[Shop.name].PedWalks.x, Config.Shops[Shop.name].PedWalks.y, Config.Shops[Shop.name].PedWalks.z, true) < 1.5) then	
									DrawText3Ds(Config.Shops[Shop.name].PedWalks.x, Config.Shops[Shop.name].PedWalks.y, Config.Shops[Shop.name].PedWalks.z + 1, "By zebrać, naciśnij [~r~E~w~]")
									if IsControlJustReleased(0, Keys['E']) and not IsPedInAnyVehicle(ped, false) then
										FreezeEntityPosition(ShopNPC, false)
										TaskTurnPedToFaceEntity(ShopNPC, PlayerPedId(), 0.2)
										startAnim(ped, "anim@gangops@facility@servers@bodysearch@", "player_search")
										exports['progressBarss']:startUI((6500), "Przeszukujesz")
										Citizen.Wait(6500)
										ClearPedTasks(ShopNPC)
										...
										ESX.TriggerServerCallback('inside-shoprobbery:payout', function(money)
											exports.pNotify:SendNotification({text = "<b>Napad na sklep</b></br>Zebrałeś "..money.."$", timeout = 4500})
		...
		if CashRegister then
			...
			if Config.Shops[Shop.name].CashRegister1 then 
				if(GetDistanceBetweenCoords(coords, Config.Shops[Shop.name].CashRegister1.x, Config.Shops[Shop.name].CashRegister1.y, Config.Shops[Shop.name].CashRegister1.z, true) < 1.5) and not Config.Shops[Shop.name].CashRegister1.robbed then	
					...
					ShowFloatingHelpNotification("~INPUT_PICKUP~ - By okraść kasetke", vector3(Config.Shops[Shop.name].CashRegister1.x, Config.Shops[Shop.name].CashRegister1.y, Config.Shops[Shop.name].CashRegister1.z))
					if IsControlJustReleased(0, Keys['E']) and not IsPedInAnyVehicle(ped2, false) then
						...
						exports['progressBarss']:startUI((4500), "Przeszukujesz")
						Citizen.Wait(4500)
			...
			if Config.Shops[Shop.name].CashRegister2 then
				...
				ShowFloatingHelpNotification("~INPUT_PICKUP~ - By okraść kasetke", vector3(Config.Shops[Shop.name].CashRegister2.x, Config.Shops[Shop.name].CashRegister2.y, Config.Shops[Shop.name].CashRegister2.z))
					if IsControlJustReleased(0, Keys['E']) and not IsPedInAnyVehicle(ped2, false) then
			...
			if(GetDistanceBetweenCoords(coords, Config.Shops[Shop.name].ShopPos.x, Config.Shops[Shop.name].ShopPos.y, Config.Shops[Shop.name].ShopPos.z, true) > 15.0) and (started or cassettedone < 2) then
				...
				CashRegister = false
				TriggerServerEvent('inside-shoprobbery:playerdroppedStop')
				exports.pNotify:SendNotification({text = "<b>Napad na sklep</b></br>Odszedłeś za daleko", timeout = 4500})
				RemoveBlip(ShopBlip)
				DeletePed(ShopNPC)
				if Config.Shops[Shop.name].CashRegister1 then
					Config.Shops[Shop.name].CashRegister1.robbed = false
