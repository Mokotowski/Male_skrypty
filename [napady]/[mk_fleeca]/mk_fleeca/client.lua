...
AddEventHandler("utk_fh:outcome", function(oc, arg)
    ...
    elseif not oc then
        ESX.ShowHelpNotification('Nie masz karty dostępu lub skarbiec jest pusty')
    end
end)
            ...
            while true do
                ...
                if dst < 40 then
                    if not LootCheck[name].Loot1 then
                        ...
                        if dst1 < 5 then
                            DrawText3D(data.trolley1.x, data.trolley1.y, data.trolley1.z+1, "[~r~E~w~] Zbierz gotówkę", 0.40)
                    ...
                    if not LootCheck[name].Loot2 then
                        ...
                        if dst1 < 5 then
                            DrawText3D(data.trolley2.x, data.trolley2.y, data.trolley2.z+1, "[~r~E~w~] Zbierz gotówkę", 0.40)
                    ...
                    if not LootCheck[name].Loot3 then
                        ...
                        if dst1 < 5 then
                            DrawText3D(data.trolley3.x, data.trolley3.y, data.trolley3.z+1, "[~r~E~w~] Zbierz gotówkę", 0.40)
                            if dst1 < 1 and IsControlJustReleased(0, 38) then
...
AddEventHandler("utk_fh:policenotify", function(name)
    ...
    if PlayerData.job.name == "police" then
        ESX.ShowHelpNotification('Alarm w banku został uruchomiony!')      
            ...
            if PlayerData.job.name == "police" and not dooruse then
                ...
                for k, v in pairs(Doors) do
                    for i = 1, 2, 1 do
                        ...
                        if dst <= 4.0 then
                            if v[i].locked then
                                DrawText3D(v[i].txtloc[1], v[i].txtloc[2], v[i].txtloc[3], "[~r~E~w~] aby otworzyć skarbiec", 0.40)
                            elseif not v[i].locked then
                                DrawText3D(v[i].txtloc[1], v[i].txtloc[2], v[i].txtloc[3], "[~r~E~w~] aby zamknąć skarbiec", 0.40)
                            end
...
AddEventHandler("utk_fh:reset", function(name, data)
    ...
    Check[name] = false
    ESX.ShowHelpNotification('DRZWI SKARBCA ZAMKNĄ SIĘ W CIĄGU 30 SEKUND!')    
    Citizen.Wait(30000)
    ESX.ShowHelpNotification('ZAMYKANIE DRZWI SKARBCA!')
...
AddEventHandler("utk_fh:startheist", function(data, name)
    ...
    exports['progressBarsss']:startUI(2000, "Używanie karty dostępu")
    Citizen.Wait(1500)
    ...
    disableinput = false
    Citizen.Wait(1000)
    Process(UTK.hacktime, "Hakowanie")
    ESX.ShowHelpNotification('Hakowanie zakończone')
    PlaySoundFrontend(-1, "ATM_WINDOW", "HUD_FRONTEND_DEFAULT_SOUNDSET")
    TriggerServerEvent("utk_fh:toggleVault", name, false)
    startdstcheck = true
    currentname = name
    ESX.ShowHelpNotification('Masz 2min do zamknięcia skarbca')
...
function SecondDoor(data, key)
    ...
    TaskStartScenarioInPlace(ped, "PROP_HUMAN_ATM", 0, true)
    ESX.ShowHelpNotification('Użyłes karty ochronny')   
     Citizen.Wait(1500)
    ...
    ClearPedTasksImmediately(ped)
    disableinput = false
    Process(2000, "Przyznawanie dostępu")
    ESX.ShowHelpNotification('Dostęp przyznany!')
        PlaySoundFrontend(-1, "ATM_WINDOW", "HUD_FRONTEND_DEFAULT_SOUNDSET")
...
function Process(ms, text)
    exports['progressBarsss']:startUI(ms, text)
    Citizen.Wait(ms)
end
...
Citizen.CreateThread(function()
    while true do
        if not done then
            ...
            if dst < 4 then
                DrawText3D(UTK.Banks[currentname].doors.secondloc.x, UTK.Banks[currentname].doors.secondloc.y, UTK.Banks[currentname].doors.secondloc.z, "[~r~E~w~] Użyj karty ochronny", 0.40)
                if dst < 0.75 and IsControlJustReleased(0 , 38) then
                    ESX.TriggerServerCallback("utk_fh:checkSecond", function(result)
                        ...
                        elseif not result then
                            ESX.ShowHelpNotification('Nie masz kart')         
                            end
                    end)
                end
            end
            ...
            for k, v in pairs(UTK.Banks) do
                if not v.onaction then
                    ...
                    if dst <= 5 and not Check[k] then
                        DrawText3D(v.doors.startloc.x, v.doors.startloc.y, v.doors.startloc.z, "[~r~E~w~] Użyj karty dostępu", 0.40)
