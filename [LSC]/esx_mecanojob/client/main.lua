...
function OpenMechnaikMenu()
  ...
  TriggerEvent('esx:showAdvancedNotification', 'System', cfgme.empty, '~g~Otworzyłeś szafkę mechaników.', cfgme.icon, 7)
    ESX.UI.Menu.Open(
        ...
        {
			align    = 'center',
            title    = 'Mechanik - Opcje Mechaników',
            elements = {
...
function OpenMechanikszmatoNardzedzia()
  ...
  ESX.UI.Menu.CloseAll()
    ...
    ESX.UI.Menu.Open(
        ...
        {
			align    = 'center',
            title    = 'Bennys Motorworks - Narzędzia',
            elements = {
              {label = 'Butla z gazem', value = 'gazbottle'},
              {label = 'Części naprawcze', value = 'fixtool'},
              {label = 'Części karoserii', value = 'carotool'},
              {label = 'Palnik', value = 'blowpipe'},
              {label = 'Zestaw Naprawczy', value = 'fixkit'},
              {label = 'Zestaw Karoserii', value = 'carokit'}
            }
        },
        function(data, menu)
...
function PrzebierzSzatnia(styl)
  TriggerEvent('skinchanger:getSkin', function(skin)
    if skin.sex == 0 then
      if Config.male ~= nil then
        TriggerEvent('skinchanger:loadClothes', skin, Config.male)
      end
    else
      if Config.female ~= nil then
        TriggerEvent('skinchanger:loadClothes', skin, Config.female)
      end
    end
  end)
end
...
Citizen.CreateThread(function()
	local blip = AddBlipForCoord(-35.01, -1052.16, 28.4)
