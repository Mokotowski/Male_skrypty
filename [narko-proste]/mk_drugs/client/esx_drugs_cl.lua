...
AddEventHandler('esx_urp:hasEnteredMarker', function(zone)
...
AddEventHandler('esx_urp:hasExitedMarker', function(zone)
	CurrentAction = nil
	ESX.UI.Menu.CloseAll()

	TriggerServerEvent('esx_urp:stopHarvestCoke')
	TriggerServerEvent('esx_urp:stopTransformCoke')
	TriggerServerEvent('esx_urp:stopHarvestMeth')
	TriggerServerEvent('esx_urp:stopTransformMeth')
	TriggerServerEvent('esx_urp:stopHarvestWeed')
	TriggerServerEvent('esx_urp:stopTransformWeed')
	TriggerServerEvent('esx_urp:stopHarvestOpium')
	TriggerServerEvent('esx_urp:stopTransformOpium')
end)
...
RegisterNetEvent('esx_urp:ReturnInventory')
AddEventHandler('esx_urp:ReturnInventory', function(cokeNbr, cokepNbr, methNbr, methpNbr, weedNbr, weedpNbr, opiumNbr, opiumpNbr, jobName, currentZone)
	cokeQTE	   = cokeNbr
	...
	myJob		 = jobName
	TriggerEvent('esx_urp:hasEnteredMarker', currentZone)
end)
...
Citizen.CreateThread(function()
	while true do
		...
		if isInMarker and not hasAlreadyEnteredMarker then
			...
			TriggerServerEvent('esx_urp:GetUserInventory', currentZone)
		end

		if not isInMarker and hasAlreadyEnteredMarker then
			hasAlreadyEnteredMarker = false
			TriggerEvent('esx_urp:hasExitedMarker', lastZone)
		end

		if isInMarker and isInZone then
			TriggerEvent('esx_urp:hasEnteredMarker', 'exitMarker')
		end
	end
end)
		...
		if CurrentAction ~= nil then
			...
			if IsControlJustReleased(0, Keys['E']) then
				...
				if CurrentAction == 'exitMarker' then
					...
					TriggerEvent('esx_urp:freezePlayer', false)
					TriggerEvent('esx_urp:hasExitedMarker', lastZone)
					Citizen.Wait(15000)
				elseif CurrentAction == 'CokeField' then
					TriggerServerEvent('esx_urp:startHarvestCoke')
				elseif CurrentAction == 'CokeProcessing' then
					TriggerServerEvent('esx_urp:startTransformCoke')
				elseif CurrentAction == 'CokeDealer' then
					TriggerServerEvent('esx_urp:startSellCoke')
				elseif CurrentAction == 'MethField' then
					TriggerServerEvent('esx_urp:startHarvestMeth')
				elseif CurrentAction == 'MethProcessing' then
					TriggerServerEvent('esx_urp:startTransformMeth')
				elseif CurrentAction == 'MethDealer' then
					TriggerServerEvent('esx_urp:startSellMeth')
				elseif CurrentAction == 'WeedField' then
					TriggerServerEvent('esx_urp:startHarvestWeed')
				elseif CurrentAction == 'WeedProcessing' then
					TriggerServerEvent('esx_urp:startTransformWeed')
				elseif CurrentAction == 'WeedDealer' then
					TriggerServerEvent('esx_urp:startSellWeed')
				elseif CurrentAction == 'OpiumField' then
					TriggerServerEvent('esx_urp:startHarvestOpium')
				elseif CurrentAction == 'OpiumProcessing' then
					TriggerServerEvent('esx_urp:startTransformOpium')
				elseif CurrentAction == 'OpiumDealer' then
					TriggerServerEvent('esx_urp:startSellOpium')
				else
					isInZone = false -- not a esx_urp zone
				end
				
				if isInZone then
					TriggerEvent('esx_urp:freezePlayer', true)
				end
...
RegisterNetEvent('esx_urp:freezePlayer')
AddEventHandler('esx_urp:freezePlayer', function(freeze)
