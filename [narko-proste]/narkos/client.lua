...
AddEventHandler('outlawNotifye', function(id ,msg)
  ...
  ESX.ShowAdvancedNotification('Zgłoszenie', 'Sprzedaż Narkotyków', msg, mugshotStr, 1)
...
--while true do
  --Wait(10)
RegisterNetEvent('tost:narkos')
AddEventHandler('tost:narkos', function()
   ...
   local pos = GetEntityCoords(ped)
    ...
    local distance = GetDistanceBetweenCoords(pos.x, pos.y, pos.z, playerloc['x'], playerloc['y'], playerloc['z'], true)
			local distanceFromCity = GetDistanceBetweenCoords(Config.Center.x, Config.Center.y, Config.Center.z, playerloc['x'], playerloc['y'], playerloc['z'], true)
              ...
              if distance <= 5 and ped  ~= GetPlayerPed(-1) and ped ~= oldped then
                TriggerServerEvent('checkD')
                if has == true then
                  drawTxt(0.90, 1.40, 1.0,1.0,0.4, "Naciśnij ~g~E ~w~, aby spróbować sprzedać narkotyki", 255, 255, 255, 255)
...
AddEventHandler('animation', function()
  ...
  while (not HasAnimDictLoaded("amb@prop_human_bum_bin@idle_b")) do Citizen.Wait(5000) end
