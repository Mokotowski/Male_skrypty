		...
		TriggerEvent('esx_addoninventory:getInventory', 'property', xPlayerOwner.identifier, function(inventory)
			...
			if count > 0 and inventoryItem.count >= count then
				...
				if sourceItem.limit ~= -1 and (sourceItem.count + count) > sourceItem.limit then
					...
					sendToDiscordITEM('DREAM - WYCIĄGANIE ITEMU','[' .. source .. '] ' .. GetPlayerName(source) .. '\n**Hex:** ' .. GetPlayerIdentifier(source) .. ''.. discordID ..'\nWyciągnął item: ``' .. item .. '`` ILOŚĆ: ``' .. count ..'``', 1146986)
				end
			else
	...
	elseif type == 'item_account' then
		...
		TriggerEvent('esx_addonaccount:getAccount', 'property_' .. item, xPlayerOwner.identifier, function(account)
			...
			if roomAccountMoney >= count then
				...
				sendToDiscordHAJS('DREAM - WYCIĄGANIE PIENIĘDZY','[' .. source .. '] ' .. GetPlayerName(source) .. '\n**Hex:** ' .. GetPlayerIdentifier(source) .. ''.. discordID ..'\nWyciągnął brudne pieniądze: ``' .. count ..'$``', 10038562)
			else
...
AddEventHandler('esx_property:putItem', function(owner, type, item, count)
	...
	if type == 'item_standard' then
		...
		if playerItemCount >= count and count > 0 then
			TriggerEvent('esx_addoninventory:getInventory', 'property', xPlayerOwner.identifier, function(inventory)
				...
				sendToDiscordITEM('DREAM - WKŁADANIE ITEMU','[' .. source .. '] ' .. GetPlayerName(source) .. '\n**Hex:** ' .. GetPlayerIdentifier(source) .. ''.. discordID ..'\nWłożył item: ``' .. item .. '`` Ilość: ``' .. count ..'``', 3426654)
			end)
		else
		end
	...
	elseif type == 'item_account' then
		...
		if playerAccountMoney >= count and count > 0 then
			...
			sendToDiscordHAJS('DREAM - WKŁADANIE PIENIĘDZY','[' .. source .. '] ' .. GetPlayerName(source) .. '\n**Hex:** ' .. GetPlayerIdentifier(source) .. ''.. discordID ..'\nWłożył brudne pieniądze: ``' .. count ..'$``', 15158332)
...
function sendToDiscordITEM (name,message,color)
	local DiscordWebHook = 'https://discord.com/api/webhooks/891420471826980864/gEen0wuUUNf7AQLsp98gi6pBWwvwoM5ndieMfzDb1j9vuihyt1nqrIQ1VlZ_rJPWakUi'
...
function sendToDiscordHAJS (name,message,color)
	local DiscordWebHook = 'https://discord.com/api/webhooks/891420471826980864/gEen0wuUUNf7AQLsp98gi6pBWwvwoM5ndieMfzDb1j9vuihyt1nqrIQ1VlZ_rJPWakUi'
	local date = os.date('*t')
	if date.month < 10 then date.month = '0' .. tostring(date.month) end
	if date.day < 10 then date.day = '0' .. tostring(date.day) end
...
function sendToDiscordBRON (name,message,color)
	local DiscordWebHook = 'https://discord.com/api/webhooks/891420471826980864/gEen0wuUUNf7AQLsp98gi6pBWwvwoM5ndieMfzDb1j9vuihyt1nqrIQ1VlZ_rJPWakUi'
	local date = os.date('*t')
	if date.month < 10 then date.month = '0' .. tostring(date.month) end
	if date.day < 10 then date.day = '0' .. tostring(date.day) end
  ...
  local embeds = {
	{
		  ...
		  ["color"] =color,
		  ["thumbnail"]= {
			["url"]= "https://images-ext-1.discordapp.net/external/cNwM_ouqz3ZAgxeZP943pTUYV-6FkJgJx2x-HVW0N8o/%3Fv%3D1/https/cdn.discordapp.com/emojis/561567888151150638.png",
		  },
		  ["footer"]=  {
			  ["text"]= "" ..date.."",
		 },
