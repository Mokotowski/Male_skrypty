				...
				if CollectionAction == 'collection' then
					...
					if dist <= 3.5 then
						if currentbag > 0 then
							...
							trashcollection = false
							truckdeposit = false
							ESX.ShowNotification("Czyszczenie ukonczone wroc do pojazdu")
...
Citizen.CreateThread(function()
	while true do
		...
		if truckdeposit then
			...
			dist = Vdist(plyCoords.x, plyCoords.y, plyCoords.z,taillight_r.x - newposx , taillight_r.y - newposy, taillight_r.z)
			if dist <= 2.0 then
			ESX.Game.Utils.DrawText3D(vector3(taillight_r.x - newposx, taillight_r.y - newposy,taillight_r.z + 0.50), "[~g~E~s~] Wyrzuc smieci do smieciarki", 1.0)
			end
		end
		...
		if trashcollection then
			...
			if dist <= 5.0 then
				if currentbag == 0 then
					ESX.Game.Utils.DrawText3D(trashcollectionpos + vector3(0.0, 0.0, 1.0), "[~g~E~s~] Wyczysc Smietnik", 1.0)				
				else
					ESX.Game.Utils.DrawText3D(trashcollectionpos + vector3(0.0, 0.0, 1.0), "[~g~E~s~] Zbieranie smieci ["..currentbag.."/"..bagsoftrash.."]", 1.0)
				end
			end
		end
...
if PlayerData.job ~= nil and PlayerData.job.name == 'garbage' then
	...
	AddTextComponentString('Smieciarz')
